...
contract PaymentSplit{
    ...
    event Join(address indexed from, address indexed refer);  //会入加入事件
    ...
    // 计算一个账户能够领取的eth
    function releasable(address _account) private  view returns (uint256) {
    ...
    // 根据受益人地址`_account`, 分账合约总收入`_totalReceived`和该地址已领取的钱`_alreadyReleased`，计算该受益人现在应分的`ETH`。
